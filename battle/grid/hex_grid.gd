...
## can run headless. HexGrid3D builds visuals from this; nothing here depends on visuals.

## Scene-tree group the runtime HexGrid3D node registers under, so units can find it.
const GROUP := &"hex_grid"

@export var layout: HexLayout
@export var tiles: Dictionary = {} ## Vector2i(q, r) -> HexTile


func _init(p_layout: HexLayout = null) -> void:
	# A loaded .tres overrides this afterwards via the exported `layout` property.
	if layout == null:
		layout = p_layout if p_layout != null else HexLayout.new()
...
func get_neighbors(coord: Vector2i) -> Array[Vector2i]:
	return layout.get_neighbors(coord)


## Walking-surface position (top centre of the tile), used to place units.
func surface_position(coord: Vector2i) -> Vector3:
	var p := layout.center_of(coord)
	p.y = layout.top_y(get_height(coord))
	return p
...
func edge_obstruction_height(a: Vector2i, b: Vector2i) -> int:
	...
	if tb != null:
		result = maxi(result, tb.effective_face_height(layout.opposite_direction(dir)))
	return result
...
func _direction_to(a: Vector2i, b: Vector2i) -> int:
	...
	for d in HexLayout.DIRECTIONS.size():
		if HexLayout.DIRECTIONS[d] == delta:
			return d
	return -1
...
## Can a unit with `profile` move directly from adjacent tile a to b?
...
func can_traverse(a: Vector2i, b: Vector2i, profile: MovementProfile) -> bool:
	if not has_tile(a) or not has_tile(b):
		return false
	if is_wall_between(a, b):
		return profile.can_climb_walls # false for now -> walls block entirely
	var dh := get_height(b) - get_height(a)
	if dh > profile.max_step_up:
		return false
	if -dh > profile.max_step_down:
		return false
...
## Cost to ENTER coord (driven by terrain; falls back to the profile default).
func enter_cost(coord: Vector2i, profile: MovementProfile) -> float:
	var t: HexTile = tiles.get(coord, null)
	if t != null and t.terrain != null:
		return t.terrain.move_cost
	return profile.default_move_cost
...
func get_reachable(start: Vector2i, budget: float, profile: MovementProfile, blocked: Dictionary = {}) -> Dictionary:
	var costs := {start: 0.0}
	var visited := {}
	...
	while not pq.is_empty():
		...
		for next in get_neighbors(current):
			if blocked.has(next) or not can_traverse(current, next, profile):
				continue
			var new_cost: float = current_cost + enter_cost(next, profile)
			if new_cost > budget:
				continue
...
func find_path(start: Vector2i, goal: Vector2i, profile: MovementProfile, blocked: Dictionary = {}) -> Array[Vector2i]:
	...
	while not pq.is_empty():
		...
		for next in get_neighbors(current):
			...
			if blocked.has(next) and next != goal:
				continue
			if not can_traverse(current, next, profile):
				continue
			var tentative: float = current_g + enter_cost(next, profile)
...
static func generate_rectangular(width: int, rows: int, max_tile_height: int,
		p_layout: HexLayout = null, terrains: Array = [],
		wall_chance: float = 0.0, max_wall_height: int = 0, rng_seed: int = 0) -> HexGrid:
	var lay: HexLayout = p_layout if p_layout != null else HexLayout.new()
	...
	return _build_random_tile_from_coords(coords, max_tile_height, lay, terrains, wall_chance, max_wall_height, rng)
...
static func generate_circular(radius: int, max_tile_height: int,
		p_layout: HexLayout = null, terrains: Array = [],
		wall_chance: float = 0.0, max_wall_height: int = 0, rng_seed: int = 0) -> HexGrid:
	var lay: HexLayout = p_layout if p_layout != null else HexLayout.new()
	var rng := RandomNumberGenerator.new()
...
static func _build_random_tile_from_coords(coords: Array[Vector2i],
		max_tile_height: int,
		lay: HexLayout, terrains: Array,
		wall_chance: float, max_wall_height: int, rng: RandomNumberGenerator) -> HexGrid:
	var grid := HexGrid.new(lay)
	var pool: Array = terrains if not terrains.is_empty() else make_default_terrains()
 
	for coord: Vector2i in coords:
		var tile := HexTile.new()
		tile.coord = coord
		tile.height = rng.randi_range(0, max_tile_height)
		tile.terrain = pool[rng.randi_range(0, pool.size() - 1)]
		grid.set_tile(tile)
	...
	if wall_chance > 0.0 and max_wall_height > 0:
		for coord: Vector2i in grid.tiles:
			...
			for d in HexLayout.DIRECTIONS.size():
				var neighbor := lay.get_neighbor(coord, d)
...
## A small starter palette. move_cost stays >= 1 so A* remains optimal.
static func make_default_terrains() -> Array:
	var soil := TerrainType.new()
	soil.id = &"soil"; soil.display_name = "Soil"
	soil.color = Color(0.45, 0.36, 0.27); soil.move_cost = 1.0

	var sand := TerrainType.new()
	sand.id = &"sand"; sand.display_name = "Sand"
	sand.color = Color(0.83, 0.74, 0.49); sand.move_cost = 1.0

	var snow := TerrainType.new()
	snow.id = &"snow"; snow.display_name = "Snow"
	snow.color = Color(0.92, 0.95, 0.98); snow.move_cost = 1.0

	var bush := TerrainType.new()
	bush.id = &"bushes"; bush.display_name = "Bushes"
	bush.color = Color(0.30, 0.50, 0.24); bush.move_cost = 1.0;
	bush.blocks_vision = true

	return [soil, sand, snow, bush]
